...
@property int waveflag;
@property int signflag;
@end
...
- (UARTPeripheral *) initWithPeripheral:(CBPeripheral*)peripheral delegate:(id<UARTPeripheralDelegate>) delegate
{
    ...
    if (self = [super init])
    {
        ...
        for (int i=0; i<10; i++) [_values addObject:[NSNumber numberWithInt:10]];
        _waveflag = 0;
        _signflag = 0;
        _stepcount = 0;
    }
...
- (void) peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    //values[t-1] = value;
    ((UILabel *)(_valueLabels[t-1])).text = [NSString stringWithFormat:@"%d", value];
    //((UIImageView *)(_dots[t-1])).alpha = rate+0.1;
    UIImageView * v = (UIImageView *)[_dots objectAtIndex:t-1];
    if (value>5) v.layer.contents = (id)[UIImage imageNamed:@"dot_light"].CGImage;
    else v.layer.contents = (id)[UIImage imageNamed:@"dot"].CGImage;
    
    //NSLog(@"index %d", t-1);
    
    [_chart.yArray[0] replaceObjectAtIndex:t-1 withObject:[NSString stringWithFormat:@"%d", value]];
    [_chart showInTheViewWirthAnimation:NO];
    
    //v.alpha = rate;
    
    _values[t-1] = [NSNumber numberWithInt:value];
    ...
    //NSLog(@"len: %d %@", [_dots count], v);
    //NSLog(@"charactor: %@ %x %d:%d %f %f", characteristic.value, val, t, value, rate, v.alpha);
    //((UILabel *)(_valueLabels[t-1])).text = [NSString stringWithFormat:@"%@", characteristic.value];
...
- (void)addValueLabel:(UILabel *)l {
    [_valueLabels addObject:l];
}

- (void)addDot:(UIImageView *)d {
    //d.alpha = 0.1;
    //NSLog(@"%f", d.alpha);
    [_dots addObject:d];
    //NSLog(@"%@:%@", d, [_dots lastObject]);
}
